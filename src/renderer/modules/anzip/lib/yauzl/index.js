...
import fd_slicer from '../fd-slicer/index';
import { crc32 } from '../buffer-crc32/index';
    ...
    readAndAssertNoEof(reader, buffer, 0, bufferSize, bufferReadStart, function (err){
        ...
        for (var i = bufferSize - eocdrWithoutCommentSize; i >= 0; i -= 1) {
            ...
            readAndAssertNoEof(reader, zip64EocdlBuffer, 0, zip64EocdlBuffer.length, zip64EocdlOffset, function (err){
                ...
                readAndAssertNoEof(reader, zip64EocdrBuffer, 0, zip64EocdrBuffer.length, zip64EocdrOffset, function (
                    err
                ){
                    if (err) return callback(err);
    ...
    readAndAssertNoEof(self.reader, buffer, 0, buffer.length, self.readEntryCursor, function (err){
        ...
        if (signature !== 0x02014b50)
            return emitErrorAndAutoClose(
                self,
                new Error('invalid central directory file header signature: 0x' + signature.toString(16))
            );
        // 4 - Version made by
            ...
            if (
                ...
                if (entry.uncompressedSize === 0xffffffff) {
                    if (index + 8 > zip64EiefBuffer.length) {
                        return emitErrorAndAutoClose(
                            self,
                            new Error('zip64 extended information extra field does not include uncompressed size')
                        );
                    }
                ...
                if (entry.compressedSize === 0xffffffff) {
                    if (index + 8 > zip64EiefBuffer.length) {
                        return emitErrorAndAutoClose(
                            self,
                            new Error('zip64 extended information extra field does not include compressed size')
                        );
                    }
                ...
                if (entry.relativeOffsetOfLocalHeader === 0xffffffff) {
                    if (index + 8 > zip64EiefBuffer.length) {
                        return emitErrorAndAutoClose(
                            self,
                            new Error('zip64 extended information extra field does not include relative header offset')
                        );
                    }
...
AssertByteCountStream.prototype._flush = function (cb){
    if (this.actualByteCount < this.expectedByteCount) {
        var msg =
            'not enough bytes in the stream. expected ' + this.expectedByteCount + '. got only ' + this.actualByteCount;
        return cb(new Error(msg));
    }
    cb();
};
...
export default {
  open, fromFd, fromBuffer, fromRandomAccessReader, dosDateTimeToDate, validateFileName, ZipFile, Entry, RandomAccessReader
};
